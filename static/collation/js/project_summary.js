...
projectSummary = (function() {
    ...
    _addHandlers = function() {
        ...
        $('#chapter').on('change', function(e) {
            ...
            if (chapter === 'all') {
                criteria = {
                    'project__id': document.getElementById('project__id').value,
                    'status': 'approved',
                    'limit': 100000000000,
    ...
    _caculateProgress = function(projectData, chapter) {
        ...
        showSummaryLoadingOverlay();
        details = {};
        expectedUnitsCriteria = {
            'transcription_identifier': projectData.basetext,
            'limit': 100000000000,
            '_fields': 'transcription_identifier,context'
        };
        ...
        api.getItemsFromDatabasePromise('transcriptions', 'collationunit', expectedUnitsCriteria).then(function(units) {
            var totalUnits, regularisedCriteria;
            ...
            regularisedCriteria = {
                'project__id': projectData.id,
                'status': 'regularised',
                'user__id': projectData.managing_editor,
                'limit': 100000000000,
                '_fields': 'status,context,created_time,last_modified_time'
            };
            if (chapter !== undefined) {
                regularisedCriteria.chapter_number = chapter;
            }
            api.getItemsFromDatabasePromise('collation', 'collation', regularisedCriteria).then(function(regularised) {
                var setCriteria;
                setCriteria = {
                    'project__id': projectData.id,
                    'status': 'set',
                ...
                if (chapter !== undefined) {
                    setCriteria.chapter_number = chapter;
                }
                api.getItemsFromDatabasePromise('collation', 'collation', setCriteria).then(function(set) {
                    var orderedCriteria;
                    orderedCriteria = {
                        'project__id': projectData.id,
                        'status': 'ordered',
                    ...
                    if (chapter !== undefined) {
                        orderedCriteria.chapter_number = chapter;
                    }
                    api.getItemsFromDatabasePromise('collation', 'collation', orderedCriteria).then(function(ordered) {
                        ...
                        api.getItemsFromDatabasePromise('collation', 'collation', approvedCriteria).then(function(approved) {
                            ...
                            api.getItemsFromDatabasePromise('collation', 'collation', editedCriteria).then(function(edited) {
                                ...
                                counts.regularised = regularised.count;
                                counts.set = set.count;
                                counts.ordered = ordered.count;
                                counts.approved = approved.count;
                                counts.edited = edited.count;
                                if (chapter != undefined) {
                                    details = _getDetails(regularised, details);
                                    details = _getDetails(set, details);
                                    details = _getDetails(ordered, details);
